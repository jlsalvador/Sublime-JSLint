...
class JslintCommand(sublime_plugin.TextCommand):
    ...
    def handel_output(self, output, temp_file_path):
        ...
        JslintGlobalStore.reset()
        JslintEventListeners.reset()
        self.view.erase_regions("jslint_errors")

        regions = []
        menuitems = []

        # For each line of jslint output (errors, warnings etc.) add a region
    ...
    def file_unsupported(self):
        ...
        has_js_or_html_extension = file_path and bool(re.search(r'\.(jsm?|html?)$', file_path))
    ...
    def on_quick_panel_selection(self, index):
        ...
        region = JslintGlobalStore.errors[index][0]
        ...
        self.view.erase_regions("jslint_selected")
        self.view.add_regions("jslint_selected", [region], "meta")


class JslintGlobalStore:
...
class JslintEventListeners(sublime_plugin.EventListener):
    ...
    def on_modified(self, view):
        ...
        # Re-run the jslint command after a second of inactivity after the view
        ...
        self.timer = Timer(timeout, lambda: view.window().run_command("jslint", {"show_panel": False}))
        self.timer.start()
    ...
    def on_post_save(view):
        ...
        if PluginUtils.get_pref("lint_on_save"):
            view.window().run_command("jslint", {"show_panel": False})
    ...
    def on_load(view):
        ...
        if PluginUtils.get_pref("lint_on_load"):
            ...
            v.run_command("jslint", {"show_panel": False})

    @staticmethod
    def on_selection_modified(view):
        caret_region = view.sel()[0]

        for message_region, message_text in JslintGlobalStore.errors:
...
class JslintSetLintingPrefsCommand(sublime_plugin.TextCommand):

    def run(self, edit):
        PluginUtils.open_config_rc(self.view.window())


class JslintSetPluginOptionsCommand(sublime_plugin.TextCommand):

    def run(self, edit):
        PluginUtils.open_sublime_settings(self.view.window())


class JslintSetKeyboardShortcutsCommand(sublime_plugin.TextCommand):
...
class JslintSetNodePathCommand(sublime_plugin.TextCommand):

    def run(self, edit):
        PluginUtils.open_sublime_settings(self.view.window())


class JslintClearAnnotationsCommand(sublime_plugin.TextCommand):
